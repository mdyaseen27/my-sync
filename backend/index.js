...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", process.env.FRONTEND_URL || 'http://localhost:5173', 'wss:', 'ws:'],
      fontSrc: ["'self'"],
      objectSrc: ["'none'"],
      frameAncestors: ["'none'"],
      baseUri: ["'self'"],
...
app.post('/api/sessions/:sessionId/join', async (req, res) => {
  try {
    ...
    if (error || !session) {
      recordPinAttempt(ip, sessionId, false);
...
app.get('/api/sessions/:sessionId', async (req, res) => {
  try {
    ...
    const { data: session, error: sessionError } = await supabase
      .from('sessions')
      .select('id, created_at, expires_at, last_active_at')
      .eq('id', sessionId)
      .single();
    ...
    const { data: devices, error } = await supabase
      .from('devices')
      .select('id, device_fingerprint, joined_at, last_seen_at')
  ...
  try {
    ...
    const { data: item, error } = await supabase
      ...
      .insert({
        ...
        iv,
        created_at: now.toISOString(),
        expires_at: itemExpiresAt.toISOString()
      })
      .select()
      .single();
    ...
    io.to(sessionId).emit('clipboard-item', {
      id: item.id,
      type: item.type,
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
    io.to(socket.sessionId).emit('device-left', { deviceId: socket.deviceId });
  });
