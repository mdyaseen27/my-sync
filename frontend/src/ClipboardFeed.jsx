...
export default function ClipboardFeed({ 
  ...
  useEffect(() => {
    ...
    return () => {
      feed.removeEventListener('dragover', handleDragOver);
      feed.removeEventListener('dragleave', handleDragLeave);
      feed.removeEventListener('drop', handleDrop);
      feed.removeEventListener('paste', handlePaste);
    };
  ...
  return (
    <div 
      className={`clipboard-feed ${dragging ? 'dragging' : ''}`}
      ref={feedRef}
      tabIndex={0}
      role="region"
      aria-label="Clipboard feed"
    >
      <div className="feed-header">
        <h3>Clipboard Feed</h3>
      ...
      {items.length === 0 ? (
        <div className="empty-state">
          ...
          <p className="empty-hint">Copy text or drag an image to get started</p>
        </div>
      ) : (
...
function ClipboardItem({ item, pin, salt, onCopyText, onCopyImage }) {
    ...
    return (
      <li className="item item-text">
        <div className="item-content">
          <pre className="item-text-content">{item.ciphertext}</pre>
        </div>
  ...
  return (
    <li className="item item-image">
      <div className="item-content">
        {previewUrl ? (
          <img src={previewUrl} alt="Synced image" className="item-image-preview" />
        ) : (
          <div className="item-image-placeholder">Loading image...</div>
        )}
      </div>
      <div className="item-actions">
        <button 
