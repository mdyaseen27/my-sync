...
export default function App() {
  ...
  if (state === 'setup') return (
    <div className="app"><div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
      <div className="card start-card"><h2>Set Your PIN</h2><p>Choose a 6-digit PIN</p>
        <form onSubmit={handleSetupSubmit}>
          <div className="pin-inputs">{setupPin.map((d, i) => <input key={i} type="text" maxLength={1} value={d} onChange={(e) => handleSetupPinDigitChange(i, e.target.value)} pattern="[0-9]*" inputMode="numeric" /></div>)}
          </div>
          <p className="setup-hint">Confirm your PIN:</p>
          <div className="pin-inputs">{setupConfirmPin.map((d, i) => <input key={i} type="text" maxLength={1} value={d} onChange={(e) => handleSetupConfirmPinDigitChange(i, e.target.value)} pattern="[0-9]*" inputMode="numeric" /></div>)}
          </div>
  ...
  if (state === 'verify') return (
    <div className="app"><div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
      <div className="card start-card"><h2>Enter Your PIN</h2>
        <form onSubmit={handleVerifySubmit}>
          <div className="pin-inputs">{pinToVerify.map((d, i) => <input key={i} type="text" maxLength={1} value={d} onChange={(e) => handlePinDigitChange(i, e.target.value)} onKeyDown={(e) => { if (e.key === 'Backspace' && !e.target.value && i > 0) { const n = [...pinToVerify]; n[i] = ''; setPinToVerify(n); e.currentTarget.focus(); } }} pattern="[0-9]*" inputMode="numeric" autoFocus />)}</div>
          </div>
          {verifyError && <p className="error">{verifyError}</p>}
